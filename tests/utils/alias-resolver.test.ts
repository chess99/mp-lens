...
describe('AliasResolver', () => {
  ...
  const mockFile = (filePath: string, isDirectory = false, content = '{}') => {
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => p === absPath || (fs.existsSync as jest.Mock).mock.calls.some(call => call[0] === p)); // Allow multiple exists checks
  ...
  beforeEach(() => {
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((p) => { throw new Error(`ENOENT: File not found ${p}`) });
    (fs.statSync as jest.Mock).mockImplementation((p) => { throw new Error(`ENOENT: Stat failed ${p}`) });
  ...
  describe('initialize', () => {
    it('should load aliases from tsconfig.json in project root with baseUrl', () => {
      ...
      (path.relative as jest.Mock).mockImplementation((from, to) => {
           ...
           if (from === projectRoot && to === actualPath.join(baseDirPath, 'components')) return 'src/components'; // for ~components/* first target
           if (from === projectRoot && to === actualPath.join(baseDirPath, 'shared/components')) return 'src/shared/components'; // for ~components/* second target
    ...
    it('should merge aliases from tsconfig and custom config, with custom taking precedence', () => {
      ...
      (fs.existsSync as jest.Mock).mockImplementation(p => p === tsconfigPath || p === customConfigPath); // Both exist
      (fs.readFileSync as jest.Mock).mockImplementation((p) => {
    ...
    it('should handle errors during config file parsing', () => {
      ...
      expect(initialized).toBe(true); 
      expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('无法解析 tsconfig.json'));
  ...
  describe('resolve', () => {
    ...
    it('should return null if alias resolves but target does not exist (even with extensions/index)', () => {
      ...
      (fs.existsSync as jest.Mock).mockReturnValue(false);
       ...
       (fs.statSync as jest.Mock).mockImplementation(p => { throw new Error(`ENOENT stat ${p}`) });
