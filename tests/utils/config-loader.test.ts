...
jest.mock('/fake/project/root/my-config-func.js', () => jest.fn(() => Promise.resolve(mockConfig)), { virtual: true });
jest.mock('/fake/project/root/my-config-es.js', () => ({ default: mockConfig }), { virtual: true });
jest.mock('/fake/project/root/my-config-es-func.js', () => ({ default: jest.fn(() => Promise.resolve(mockConfig)) }), { virtual: true });
...
jest.mock('/fake/project/root/error.js', () => { 
...
describe('ConfigLoader', () => {
  ...
  beforeEach(() => {
    ...
    (fs.readFileSync as jest.Mock).mockImplementation((filePath) => {
        ...
        if (normalizedPath === actualPath.normalize(actualPath.resolve(projectRoot, 'my-config.json')) || 
            normalizedPath === actualPath.normalize(actualPath.join(projectRoot, 'mp-analyzer.config.json'))) {
          return JSON.stringify(mockConfig);
        }
        if (normalizedPath === actualPath.normalize(actualPath.resolve(projectRoot, 'invalid.json'))) {
          return 'invalid json{';
        }
        // Let actual require handle mocked JS/TS files via jest.mock, throw otherwise for readFileSync
        throw new Error(`Mock fs.readFileSync: ENOENT: no such file or directory, open '${filePath}'`);
      });
  ...
  it('should auto-find and load mp-analyzer.config.js if exists', async () => {
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => actualPath.normalize(p) === actualPath.normalize(jsConfigPath));
  ...
  it('should auto-find and load mp-analyzer.config.ts if .js doesnt exist', async () => {
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => actualPath.normalize(p) === actualPath.normalize(tsConfigPath)); // Only TS exists
  ...
  it('should auto-find and load mp-analyzer.config.json if .js and .ts dont exist', async () => {
    ...
    (fs.existsSync as jest.Mock).mockImplementation((p) => actualPath.normalize(p) === actualPath.normalize(jsonConfigPath)); // Only JSON exists
  ...
  it('should return null if no config file is found during auto-search', async () => {
    ...
    expect(fs.existsSync).toHaveBeenCalledWith(actualPath.resolve(projectRoot, 'mp-analyzer.config.js'));
    expect(fs.existsSync).toHaveBeenCalledWith(actualPath.resolve(projectRoot, 'mp-analyzer.config.ts'));
    expect(fs.existsSync).toHaveBeenCalledWith(actualPath.join(projectRoot, 'mp-analyzer.config.json'));
  ...
  it('should return null and log error if TS loading fails because ts-node require fails', async () => {
    ...
    jest.doMock('/fake/project/root/my-config.ts', () => { throw requireError; }, { virtual: true });
  ...
  it('should return null and log error if TS loading fails because ts-node registration fails', async () => {
      ...
      jest.doMock('ts-node', () => ({ 
          register: jest.fn(() => { throw registrationError; }) 
      }), { virtual: true });
