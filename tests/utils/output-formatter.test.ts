...
// Mock chalk module
// Simplistic mock: return plain text, maybe wrap with color name for clarity
const mockChalk = {
  level: 0, // Disable color codes
...
describe('Output Formatter', () => {
  ...
  describe('formatAsText', () => {
    ...
    it('should return specific message for zero unused files', () => {
       ...
       expect(mockChalk.green).toHaveBeenCalledWith('未发现未使用的文件。');
    });
       
    it('should group files by type correctly in text output', () => {
      // This is implicitly tested by the snapshot test above, 
      // but we can add specific checks if needed.
      const output = formatOutput(unusedFiles, options);
      // Check if type headers are present (case-insensitive due to chalk mock)
      expect(output).toContain('[cyan]WXML 文件 (1):[/cyan]');
      expect(output).toContain('[cyan]WXSS 文件 (1):[/cyan]');
      expect(output).toContain('[cyan]JS 文件 (1):[/cyan]');
      expect(output).toContain('[cyan]TS 文件 (1):[/cyan]');
      expect(output).toContain('[cyan]PNG 文件 (1):[/cyan]');
      expect(output).toContain('[cyan]UNKNOWN 文件 (1):[/cyan]'); // For no_extension_file
    });
       
    it('should handle files without extensions correctly in text output (as UNKNOWN)', () => {
      const filesWithNoExt = ['/path/to/project/some_file_no_ext'];
      const output = formatOutput(filesWithNoExt, options);
      expect(output).toContain('[cyan]UNKNOWN 文件 (1):[/cyan]');
      expect(output).toContain('[white]some_file_no_ext[/white]');
      expect(output).toMatchSnapshot(); // Use snapshot for the overall structure
    });
  });
}); 